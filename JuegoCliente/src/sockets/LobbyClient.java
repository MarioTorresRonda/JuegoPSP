...
public class LobbyClient extends Thread {
	...
	public String Join(Lobby lobby) {
		...
		if ( ip != "error") {
			main.toGame( lobby );
			GameClient client = new GameClient(main, main.getClient().getNombre(), ip );
			main.setClientGame(client);
		}
		return "1";
	}
	...
	public Lobby Create() {
		...
		if ( lobby != null) {
			GameServer server = new GameServer(main);
			main.setServerGame(server);
			Join( lobby );
		}
		return lobby;
